...
;
    ...
    protected function getNameColumn(): Column
    {
        return TextColumn::make('name')
            ->label(__('admin/sms_driver.columns.name'))
            ->sortable()
            ->searchable();
    }
    ...
    protected function getOptionProviderColumn(): Column
    {
        return TextColumn::make('options.provider')
            ->label(__('admin/sms_driver.columns.options_provider'))
            ->formatStateUsing(fn(string $state) => __('admin.sms_providers.' . SmsProvider::tryFrom($state)->value))
            ->badge();
    }
    ...
    protected function getIntroColumn(): Column
    {
        return TextColumn::make('intro')
            ->label(__('admin/sms_driver.columns.intro'))
            ->limit(60)
            ->searchable();
    }
    ...
    protected function getCreatedAtColumn(): Column
    {
        return TextColumn::make('created_at')
            ->label(__('admin/sms_driver.columns.created_at'))
            ->sortable()
            ->alignCenter()
            ->dateTime();
    }
